...
{
	/*
	 * C4 Detonator Item In game use: "Arms" C4 blocks when it right-clicks them.
	 * Right-clicking an already armed block disarms it. Innate use: "Detonates"
	 * armed C4 blocks
	 * 
	 * onItemUse: Grabs the BlockPos of targeted block, stores it in local ArrayList
	 * if its from a C4 instance, removes it from ArrayList if it is already there.
	 * This is the "arming" and "disarming" functionality.
	 * 
	 * onItemRightClick: If crouching, toggles the fuse value between 0, 3, 5, 9, 12, seconds
	 */
	@OnlyIn(Dist.CLIENT)
	private int timer = 0;
	@OnlyIn(Dist.CLIENT)
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn)
	{
		...
		tooltip.add(new StringTextComponent("§o§e  The air pressure causes damage in a larger area"));
		tooltip.add(new StringTextComponent("§o§e  than the explosion itself! Watch your fingers!"));
	}
	...
	// Right-click without crouching sets the detonator off. Crouch-clicking sets
	// the fuse time.
	@Override
	// @OnlyIn(Dist.CLIENT)
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn)
	{
		if(worldIn.isRemote) return ActionResult.resultFail(playerIn.getHeldItem(handIn));
	...
	public void detonateInstances(World world, int fuse)
	{
		...
		for(BlockPos i : c4blocks)
		{
			...
			if(world.getBlockState(i).getBlock() == Blocks.AIR) world.removeBlock(i, true);
		}
		c4blocks = new ArrayList<BlockPos>();
	}
