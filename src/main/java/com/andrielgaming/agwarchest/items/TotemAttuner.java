...
public class TotemAttuner extends Item implements IVanishable
{
	 ...
	 * ------------------------------- 
	 * Checks the tool, weapon, or armor item held in the offhand for valid enchantments. 
	 * If it has not already been "attuned", it will "attune" the gear item by doubling its 
	 * enchantment strength, or in cases of enchants that can't simply be doubled, replace them with new
	 * enchants that functionally act like doubled versions of their vanilla variants (See Enchantments package)
	 */
	@Override
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn)
	{
		...
		// Check if the enchantment map contains an AttunedEnchant and return a fail if so, gear
		// can't be attuned twice. Check first so totem won't break prematurely
		if(EnchantmentHelper.getEnchantments(tool).containsKey(EnchantInit.ATTUNED.get()))
		...
		if ((it.getGroup() == ItemGroup.TOOLS || it.getGroup() == ItemGroup.COMBAT) && (tool.isEnchanted()))
		{
			...
			for (int i = 0; i < 80; i++)
			{
				worldIn.addParticle(ParticleTypes.field_239812_C_, playerIn.getPosXRandom(1.75),
						playerIn.getPosYRandom(), playerIn.getPosZRandom(1.75), 0.0D, 0.0D, 0.0D);
				worldIn.addParticle(ParticleTypes.PORTAL, playerIn.getPosXRandom(1.75), playerIn.getPosYRandom(),
						playerIn.getPosZRandom(1.75), 0.0D, 0.0D, 0.0D);
			}
			...
			{
				if (enchants.get(entry.getKey()) > 1)
					enchants.put(entry.getKey(), entry.getValue() * 2);
				else if(enchants.get(entry.getKey()) <= 1)
					enchants.put(entry.getKey(), 2);
			}
	...
	//Takes the passed in ItemStack and applies the attuned variants for all complex enchants
	...
	private Stack<Enchantment> processComplex(ItemStack item)
	{
		...
		if (enchants.containsKey(EnchantInit.ATTUNED.get()))
			return null;
