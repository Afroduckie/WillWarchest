...
public class FluidExplosion extends Explosion
{
	   ...
	   {
	      return (IExplosionContext)(p_234894_1_ == null ? DefaultExplosionContext.INSTANCE : new EntityExplosionContext(p_234894_1_)); //TODO
	   }//Explosion
	     ...
	     boolean flag = this.mode != FluidExplosion.Mode.NONE;
	      if (true) 
	      {
	         if (!(this.size < 2.0F) && flag) 
	         {
	            ...
	            //this.world.addParticle(ParticleTypes.FLAME, this.x, this.y, this.z, 0.35D, 0.0D, 0.0D);
	         } 
	         else 
	         {
	      ...
	      if (this.causesFire) 
	      {
	         for(BlockPos blockpos2 : this.affectedBlockPositions) 
	         {
	            if (this.random.nextInt(1) == 0 && this.world.getBlockState(blockpos2).getBlock() == Blocks.AIR && this.world.getBlockState(blockpos2.down()).isOpaqueCube(this.world, blockpos2.down())) 
	            {
	               ...
	               temp = blockpos2.up();
	               if (mode == FluidExplosion.Mode.FIRE &&this.random.nextInt(2) == 0)// && world.getBlockState(temp).getBlock() == Blocks.AIR) 
	               {
	            	  //this.world.setBlockState(temp2, AbstractFireBlock.func_235326_a_(this.world, temp2));
